...
use cosm_orc::client::chain_res::ChainResponse;
...
// my attempt
// pub async fn send_msgs<I: IntoIterator<Item = Any>>(messages: I, config: &mut Config) -> Result<(), DeployError>{
...
async fn simulate_gas_fee(
    ...
    let amount = Coin {
        denom: denom.clone(),
        amount: ((gas_limit * cfg.gas_price).ceil() as u64).into(),
    };
