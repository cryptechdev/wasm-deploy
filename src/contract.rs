...
use crate::{error::{DeployError, DeployResult}, file::{Config, ContractInfo}, cosmwasm::CosmWasmClient};
...
pub async fn execute_instantiate(contract: &impl Contract) -> Result<(), DeployError> {
    ...
    let response = client.instantiate(
        code_id,
        payload, 
        &key,
        Some(contract.admin()),
